...
type Challenge = {
    id: string;
    name?: string;                // caso o payload use "name"
    ChallengeTitle?: string;      // compat com seu mock antigo
    authorName?: string;          // se existir
    Author?: string;              // compat com mock
    category?: string;
    Category?: string;            // compat com mock
    status: Status;
    Status?: Status;              // compat com mock
    createdAt?: string;
    updatedAt?: string;
    Date?: string;                // compat com mock
    visibility: Visibility;
    Visibility?: Visibility;      // compat com mock
    enterpriseId?: string;
    usersId?: string;             // criador
    createdById?: string;         // dependendo do backend
};

type Props = {
    /** Se a página tiver /company/[companyId]/history, podemos receber pra limitar admin */
    companyId?: string;
    /** Role de quem está vendo */
    role: Role;
    /** Empresa do viewer (gestor/avaliador) */
    viewerCompanyId?: string;
    /** UserId do viewer (usuario COMMON) */
    viewerUserId?: string;
};
...
export default function CompanyHistoryHistoric({
    ...
    React.useEffect(() => {
        (async () => {
            ...
            try {
                // Monta params por regra de permissão
                ...
                if (role === "admin") {
                    ...
                    params.createdById = viewerUserId;
                }

                // Tenta buscar do endpoint de histórico
                let data = await ChallengeService.showHistorical(params);

                // Fallback/garantia: mantém apenas Completed ou Archived
                const historicalOnly = (data ?? []).filter((c: any) => {
                    const s: string = (c.status ?? c.Status ?? "").toString();
                    return s === "Completed" || s === "Archived";
                });

                // Filtros adicionais client-side (caso o backend ignore algum param)
                ...
                if (role === "gestor" || role === "avaliador") {
                    ...
                    if (viewerUserId) {
                        const key = (c: any) => String(c.createdById ?? c.usersId);
                        scoped = scoped.filter((c) => key(c) === String(viewerUserId));
                    }
