...
export const DetailedScreening = ({ challangeTitle, challengeId, category, startDate, endDate, creator, visibility }: CardDetailedScreeningContentProps) => {
  ...
  const handleSave = async () => {
    if (!detailedScreening) {
      console.log("Erro: Dados da triagem não estão prontos.");
      return;
    }
    const payload: UpdateDetailedScreeningPayload | CreateDetailedScreeningPayload = {
    ...
    try {
      if (detailedScreening.id) {
        const updatedResponse = await detailedScreeningService.updateDetailedScreening(detailedScreening.id, payload);

        const savedData: ShowDetailedScreeningResponse = {
          ...updatedResponse,
          TriagemVoto: detailedScreening.TriagemVoto, 
          porcentagemMedia: detailedScreening.porcentagemMedia
        };

        setDetailedScreening(savedData);

      } else {
        const createdResponse = await detailedScreeningService.createDetailedScreening(challengeId, payload);
      ...
      {page === '1' && detailedScreening && (
        <div className="w-full flex flex-col h-full">
          ...
          <div className="flex-1 flex flex-col gap-3">
            ...
            <button
              className="mt-4 px-4 py-2 bg-[#0B2B72] text-white rounded-md text-sm self-end"
              onClick={handleSave}
            >
              Salvar Alterações
            </button>
              ...
              <div className="flex flex-col mb-6">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-col gap-[2px] mt-2">
                    {["Nenhuma", "API Externa", "API Interna", "Outro"].map((opt) => (
            ...
            <div className="mt-6 flex justify-end gap-4">
              {detailedScreening.id && (
                <button
                  type="button"
                  onClick={handleVote}
                  className="px-5 py-2 bg-[#0B2B72] text-white text-sm rounded-md self-end hover:bg-[#0A2461] transition disabled:opacity-50"
              ...
              <button
                className="mt-4 px-5 py-2 bg-[#0B2B72] text-white text-sm rounded-md self-end hover:bg-[#0A2461] transition disabled:opacity-50"
                onClick={handleSave}
                disabled={!detailedScreening}
              >
                Salvar Alterações
